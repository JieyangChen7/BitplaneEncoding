...
void test_gpu_v4(int n, int b) {
    ...
    const int batches_per_block = 8;
    const int batches_per_warp = 2;
    dim3 num_blocks(num_batches/batches_per_block);
    dim3 block_size(128);


    int dataBytes   = batch_size * sizeof(T_fp);
    int signBytes   = batch_size * sizeof(T_sfp);
    int resultBytes = b * (BATCHES_PER_BLOCK * 2) * sizeof(T_bp);
    int sharedMemSize = dataBytes + signBytes + resultBytes;
    printf("Shared Memory size: %d bytes", sharedMemSize);
    dim3 gridDim((num_batches + BATCHES_PER_BLOCK - 1) / BATCHES_PER_BLOCK);
    dim3 blockDim(batch_size);

    // Warmup GPU
    for (int i = 0; i < 10; i++) {
        gpu_v4::encode<T, T_fp, T_sfp, T_bp><<<gridDim, blockDim, sharedMemSize>>>(d_data, d_encoded_bitplanes, n, b, exp);
    }

    printf("Encode (GPU V4): \n");
    cudaDeviceSynchronize();
    auto start = high_resolution_clock::now();
    for (int i = 0; i < 10; i++) {
        gpu_v4::encode<T, T_fp, T_sfp, T_bp><<<gridDim, blockDim, sharedMemSize>>>(d_data, d_encoded_bitplanes, n, b, exp);
    ...
    printf("Decode (GPU V4): \n");
    cudaDeviceSynchronize();
    start = high_resolution_clock::now();
    for (int i = 0; i < 10; i++) {
        gpu_v4::decode<T, T_fp, T_sfp, T_bp><<<num_blocks, block_size>>>(d_decoded_data, d_encoded_bitplanes, batches_per_warp, n, b, exp);
    }
    cudaDeviceSynchronize();
    stop = high_resolution_clock::now();
    time = duration_cast<microseconds>(stop - start).count() / 10;
...
void test(int n, int b) {
    ...
    test_gpu_v4<float, T_fp, T_sfp, T_bp>(n, b);
}
