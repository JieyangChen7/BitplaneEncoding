...
template <typename T>
void print_bits(T v, int num_bits, bool reverse = false) {
...
void encode_batch(T_fp *v, T_bp *encoded, int batch_size, int b) {
...
template <typename T_fp, typename T_bp>
void decode_batch(T_fp *v, T_bp *encoded, int batch_size, int b) {
    for (int data_idx = 0; data_idx < batch_size; data_idx++) {
        T_fp buffer = 0;
        for (int bp_idx = 0; bp_idx < b; bp_idx ++) {
...
void encode (T *v, T_bp * encoded_bitplanes, int n, int b, int exp) {
...
template <typename T, typename T_fp, typename T_sfp, typename T_bp>
void decode (T *v, T_bp * encoded_bitplanes, int n, int b, int exp) {
